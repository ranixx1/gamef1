...
start_carro:
    # Posição inicial: linha 10, coluna G (índice 6 → endereço 10×512 + 6×4 = 5144
    lui   $19, 0x1001
    addi  $19, $19,5120
    # Cores fixas nos registradores
    lui   $12, 0x00FF             # $12 = vermelho 0x00FF0000
    ori   $12, $12, 0x0000

    lui   $15, 0x00FF             # $15 = amarelo da borda 0x00FFFF00
    ori   $15, $15, 0xFF00

    ori   $14, $0, 0x00FF         # $14 = verde da grama 0x00FF0000 (igual ao que você usou no início)
    sll   $14, $14, 8             # agora $14 = 0x00FF0000

    sw    $12, 0($19)             # pinta vermelho no começo

loop_carro:
    # === Lê teclado ===
    lui   $17, 0xFFFF
    lw    $18, 0($17)             # tem tecla?
    beq   $18, $0, loop_carro     # não → espera

    lw    $20, 4($17)             # $20 = código da tecla (w=119, a=97, s=115, d=100)

    # === Apaga carro antigo (pinta de VERDE, igual à grama!) ===
    sw    $14, 0($19)             # deixa verde onde estava
    ...
    j     pinta_novo              # tecla inválida → só pinta de novo

up_car:
    addi  $19, $19, -512          # sobe 1 linha
    j     checa
down_car:
    addi  $19, $19, 512           # desce 1 linha
    j     checa
left_car:
    addi  $19, $19, -4            # esquerda
    j     checa
rigth_car:
    addi  $19, $19, 4             # direita

checa:
    # Verifica se nova posição tem amarelo
    lw    $22, 0($19)
    beq   $22, $15, game_over     # TOCOU NA BORDA AMARELA → MORRE

pinta_novo:
    sw    $12, 0($19)             # pinta vermelho na nova posição
